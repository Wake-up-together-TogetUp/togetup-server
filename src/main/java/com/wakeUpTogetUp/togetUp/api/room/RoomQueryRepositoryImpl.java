...
public class RoomQueryRepositoryImpl implements RoomQueryRepository {
	...
	public List<RoomRes> findRoomsOrderedByJoinTime(Integer userId) {
		...
		Join<Room, Alarm> alarmJoin = roomJoin.join("alarms", JoinType.LEFT);
		Join<Alarm, MissionObject> missionObjectJoin = alarmJoin.join("missionObject", JoinType.LEFT);
	...
	public Optional<Room> findByInvitationCode(String invitationCode) {
		List<Room> rooms = entityManager.createQuery("SELECT r FROM Room r WHERE r.invitationCode = :invitationCode",
				Room.class)
