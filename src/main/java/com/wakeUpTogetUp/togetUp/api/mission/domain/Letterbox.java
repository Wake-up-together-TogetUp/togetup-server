...
public class Letterbox {
    ...
    public Mat letterbox(Mat image) { // 단계 제약 조건을 충족하도록 이미지 크기 조정 및 채우기, 매개 변수 기록
        int[] shape = {image.rows(), image.cols()}; // 현재 모양 [height, width]
        // Scale ratio (new / old)
        ...
        if (shape[1] != newUnpad.width || shape[0] != newUnpad.height) {
            Imgproc.resize(image, image, newUnpad, 0, 0, Imgproc.INTER_LINEAR);
        }
        ...
        // 그림을 정사각형으로 채우기
        Core.copyMakeBorder(image, image, top, bottom, left, right, Core.BORDER_CONSTANT, new org.opencv.core.Scalar(this.color));
